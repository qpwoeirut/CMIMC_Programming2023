    ...
    if round_num >= 100:
        for i, loc in enumerate(locations):
            if sum(board[loc[0]][loc[1]]) == 0:
                bomb_count[i] += 1  # very approximate
...
def flexible_pattern_score(pid: int, board: list[list[list[int]]], loc: tuple[int, int]) -> int:
    ...
    bomb_percent = 0 if sum(bomb_count) == 0 else round(bomb_count[idx] * 100 / sum(bomb_count))
    return 100 * (nearby_blast_score(board, loc[0], loc[1]) + board[loc[0]][loc[1]][pid] * 3) + bomb_percent + random.randint(0, 99)
...
def get_strategies():
    ...
    """
    strategies = [flexible_offset_pattern_strategy, flexible_offset_pattern_strategy] + [random_strategy for _ in range(3)]
